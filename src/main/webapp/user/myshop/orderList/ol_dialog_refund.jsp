...
<div data-name="${opvo.getProduct_name()}" data-img="${opvo.getImg_url()}" data-qty="${odvo.getOd_changed_qty()}" data-price="${pdvo.getCell_price()*odvo.getOd_changed_qty()}" data-option='${opvo.size} ${opvo.color}'>
	<a href="#none" class="btnNormal updateOd refund${Rf_idx }">반품신청</a>
</div>
<!-- 모달 다이얼로그 -->
<div id="myModalRefund${Rf_idx}" class="modalRf${Rf_idx}">
    <div class="modal-content">
        <div class="header">
            <h1>상품 반품 신청</h1>
            <a id="closeRf${Rf_idx}" class="close"><img src="//img.echosting.cafe24.com/skin/base/common/btn_close.gif" alt="닫기" />&times;</a>
            
        </div>
        
        <div class="contents">
            <div class="option-section">
        		<div id="modal-products-${Rf_idx}">
					<div class="product">
            ...
            <hr/>
			<div style="display:block;" id="option_products_divRf${Rf_idx}" class="{$total.total_display|display}">
				<table>
					<tbody class="option_products refund">
						<tr class="product" data-qty="${odvo.getOd_changed_qty()}">
				            ...
				            <td style="text-align:center; padding:20px 40px 20px 40px; ">
					            <div class="qty_Rf${Rf_idx}" >
								    <button type="button" class="refund qty down"><img src="//img.echosting.cafe24.com/skin/base_ko_KR/product/btn_count_down.gif"/></button>
								    <input type="text" data-max="${odvo.getOd_changed_qty()}" value="1" min="1" class="qty-input" id="qty_input_Rf${Rf_idx}" readonly style="width: 50px; text-align: center;" />
								    <button type="button" class="refund qty up"><img src="//img.echosting.cafe24.com/skin/base_ko_KR/product/btn_count_up.gif"/></button>
								</div>
							</td>
				        </tr>
		            </tbody>
            ...
            <div style="padding:20px;" id="total_amount_Rf${Rf_idx}" class="total_amount"><!-- 여기에 js로 수량 추가합니당 --></div>
            <div style="padding:20px;" class="total_amount">
            반품(환불) 사유: 
            <input id="Rf_msg_${Rf_idx}" type="text" placeholder="사유 입력" />
            </div>
            <div class="ec-base-button" module="product_action">
                <a href="#none" onclick="odRefund${Rf_idx}()"class="btnSubmitFix sizeS"><span id="btn_buy">상품 반품</span></a>
                <a href="#none" onclick="closeModalRf${Rf_idx}()" class="btnNormalFix sizeS">닫기</a>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
 	...
 	// 모달 다이얼로그 열기
    $(".updateOd.refund${Rf_idx}").on("click", function() { 

        const modal = document.getElementById("myModalRefund${Rf_idx}");

        const div = $(this).closest("div");
		
        $("#qty_input_Rf${Rf_idx}").val(1);
	
        $('.modalRf${Rf_idx}').off('click', '.qty_Rf${Rf_idx} .refund.qty.up').on('click', '.qty_Rf${Rf_idx} .refund.qty.up', function() {
        	let input = $(this).siblings("input[type='text']");
            let currentVal = parseInt(input.val());
            let prod_stock = input.data("max");

            if(currentVal >= prod_stock){
                alert("반품가능 수량은 기존 주문량을 넘을 수 없습니다.");
                input.val(prod_stock);
                return ; 
            } else {
                input.val(currentVal + 1);
                updateTotalAmountRf${Rf_idx}();
            }
        });

        $('.modalRf${Rf_idx}').off('click', '.qty_Rf${Rf_idx} .refund.qty.down').on('click', '.qty_Rf${Rf_idx} .refund.qty.down', function() {
            let input = $(this).siblings("input[type='text']");
            let currentVal = parseInt(input.val());
            if (currentVal > 1) {
                input.val(currentVal - 1);
    ...
    // 모달 다이얼로그 닫기
    $("#closeRf${Rf_idx}").on("click", function() {
    	const modal = document.getElementById("myModalRefund${Rf_idx}");
...
//총 상품금액 업데이트 함수
function updateTotalAmountRf${Rf_idx}() {
	let totalPrice = 0;
    let totalQuantity = 0;
    $("#option_products_divRf${Rf_idx} .option_products.refund .product").each(function() {
        
        let quantity = parseInt($(this).find("#qty_input_Rf${Rf_idx}").val());
    ...
    $("#total_amount_Rf${Rf_idx}").html('<strong>변경후 상품금액</strong>(수량) : <span class="total-cnt"><strong><em>' + totalPrice.toLocaleString() + '원</em></strong> (' + totalQuantity + '개)</span>');
}


function odRefund${Rf_idx}(){
	if(confirm("해당 상품을 반품하시겠습니까?")){
		if(confirm("반품 시 번복이 불가합니다.\r\n반품을 진행하시겠습니까?")){
			
			let qty = $("#qty_input_Rf${Rf_idx}").val();
			let msg = $("#Rf_msg_${Rf_idx}").val();
